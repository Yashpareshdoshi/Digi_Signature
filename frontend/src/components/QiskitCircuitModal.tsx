...
export const QiskitCircuitModal: React.FC<QiskitCircuitModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 dark:bg-black/80 backdrop-blur-sm">
      <div className="relative w-full max-w-3xl rounded-2xl bg-white border border-slate-200 dark:bg-slate-900 dark:border-cyan-500/40 shadow-2xl p-6 space-y-4 transition-colors">
        <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="px-2 py-0.5 rounded text-[10px] font-mono bg-cyan-100 text-cyan-800 border border-cyan-300 dark:bg-cyan-950 dark:text-cyan-300 dark:border-cyan-500/30">
        ...
        <div className="text-xs text-slate-600 dark:text-slate-400">
          ...
          and Bob classically controlled Pauli operations (<code className="text-cyan-700 dark:text-cyan-300 font-bold">c_if</code>)
          for state <code className="text-cyan-700 dark:text-cyan-300 font-bold">{quantumState}</code> and Bell pair{' '}
          <code className="text-cyan-700 dark:text-cyan-300 font-bold">{bellState}</code>.
        </div>

        <div className="relative p-4 rounded-xl bg-slate-950 border border-slate-800 font-mono text-[11px] text-cyan-300 overflow-x-auto whitespace-pre leading-tight max-h-96 shadow-inner">
          <button
            onClick={handleCopy}
        ...
        <div className="flex justify-end pt-2">
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-200 text-xs font-semibold transition-colors"
          >
            Close
          </button>
        </div>
