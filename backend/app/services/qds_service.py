...
class QDSService:
    ...
    def create_signature(
        ...
        2. Prepares input Pauli eigenstate |psi> on Alice's qubit via QuantumBackend.
        ...
        # Step 2: Prepare Quantum State via Backend
        input_statevector = backend.get_pauli_state(quantum_state)

        # Step 3 & 4: Simulate Teleportation with Pauli Correction via Backend
        teleport_result = backend.teleport(
            input_state=input_statevector,
            bell_state_name=bell_state
        )

        # Step 5: Perform Projective Measurement on Bob's Genuinely Recovered State
        expected_outcome = get_expected_outcome(quantum_state, measurement_basis)
        recovered_state = teleport_result.get("recovered_statevector")
        if recovered_state is None:
            recovered_state = backend.get_pauli_state(quantum_state)
        ...
        # Step 6: Persist in SQLite DB with complete quantum token metadata
        sig_record = Signature(
            ...
            nonce_consumed=0,
            status="GENERATED",
            teleport_bits=teleport_result["classical_bits"],
            pauli_correction=teleport_result["pauli_correction"],
            teleport_fidelity=teleport_result["fidelity"],
